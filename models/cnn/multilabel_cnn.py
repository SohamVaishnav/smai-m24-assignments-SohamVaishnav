...
class Model_trainer:
    ...
    def trainer(self, X_train, y_train, X_valid, y_valid, epochs) -> None:
        ...
        for epoch in range(epochs):
            ...
            acc, loss, y_pred_train = self.evaluate(X_train, y_train, False)
            acc_train.append(acc)
            loss_train.append(loss)
            acc, loss, y_pred_val = self.evaluate(X_valid, y_valid, False)
            acc_valid.append(acc)
            loss_valid.append(loss)

        # plt.figure(figsize=(10, 5))
        # plt.plot(acc_train, label = 'Train Accuracy')
        # plt.plot(acc_valid, label = 'Valid Accuracy')
        # plt.legend()
        # plt.xlabel('Epochs')
        # plt.ylabel('Accuracy')
