...
class MultiMNISTDataset(object):
    ...
    def load_mnist_data(self, task: str) -> None:
        ...
        for root, _, files in os.walk(self._test_path):
            ...
            if (task == 'num_digits'):
                ...
                if ([len(file.split('_')) > 1 for file in files] == [True for file in files]):
                    self._test_labels.extend([int(file.split('_')[0])+1 for file in files])
                else:
                    self._test_labels.extend([0 for file in files])
    ...
    def getLabels(self, task: str) -> None:
        ...
        if (task == 'num_digits'):
            ...
            self._labels = np.zeros((len(labels), 33))
            for i in range(len(labels)):
                temp = str(labels[i]-1)
                if (len(temp) == 1 or temp == '-1'):
                    self._labels[i][int(temp)+1] = 1
                    self._labels[i][11] = 1
                    self._labels[i][22] = 1
                elif (len(temp) == 2 and temp != '-1'):
                    self._labels[i][int(temp[0])+1] = 1
                    self._labels[i][11+int(temp[1])+1] = 1
                    self._labels[i][22] = 1
                elif (len(temp) == 3):
                    self._labels[i][int(temp[0])+1] = 1
                    self._labels[i][11+int(temp[1])+1] = 1
                    self._labels[i][22+int(temp[2])+1] = 1
        return self._labels
    ...
    def OneHotEncoder(self, curr_labels: list) -> np.ndarray:
        ...
        for i in range(len(curr_labels)):
            temp = str(curr_labels[i]-1)
            if (len(temp) == 1 or temp == '-1'):
                labels[i][int(temp)+1] = 1
                labels[i][11] = 1
                labels[i][22] = 1
            elif (len(temp) == 2 and temp != '-1'):
                labels[i][int(temp[0])+1] = 1
                labels[i][11+int(temp[1])+1] = 1
                labels[i][22] = 1
            elif (len(temp) == 3):
                labels[i][int(temp[0])+1] = 1
                labels[i][11+int(temp[1])+1] = 1
                labels[i][22+int(temp[2])+1] = 1
        return labels
